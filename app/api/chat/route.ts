...
export async function POST(request: Request) {
  try {
    ...
    const knowledge = content.knowledge
      .map((item) => `Tema: ${item.topic}\nPregunta: ${item.question}\nRespuesta: ${item.answer.slice(0, 6000)}`)
      .join("\n\n")
      .slice(0, 55_000);
    ...
    return NextResponse.json({ reply: outputText || fallback(message, content.knowledge.map((item) => item.answer)) });
